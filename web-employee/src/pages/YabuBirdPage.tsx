...
const NETWORK_SYNC_MS = 240
const LOCATION_REFRESH_MS = 12000
const MENU_PRESS_MS = 90
const MENU_RELEASE_MS = 120
...
function getVisiblePipes(seed: number, elapsedMs: number): PipeSprite[] {
  ...
  for (let index = startIndex; index < startIndex + 7; index += 1) {
    ...
    const gapCenter = 66 + seededUnit(seed, index) * Math.max(0, PLAY_HEIGHT - 126)
...
function formatRoomSlots(room: YabuBirdRoom): string {
  return `${Math.min(room.player_count, ROOM_LIST_CAPACITY)}/${ROOM_LIST_CAPACITY}`
}
...
function drawWorldShift(ctx: CanvasRenderingContext2D, theme: WorldTheme, intensity: number): void {
  ...
  ctx.globalAlpha = Math.min(0.2, intensity * 0.22)
...
export function YabuBirdPage() {
  ...
  const menuRoomsRef = useRef<YabuBirdRoom[]>([])
  ...
  const roomList = useMemo(
    () =>
      menuLiveRooms
        .filter((entry) => entry.room_type === 'PARTY')
        .sort(
          (left, right) =>
            right.player_count - left.player_count || (left.share_code ?? '').localeCompare(right.share_code ?? ''),
        ),
    [menuLiveRooms],
  )
  ...
  function resetToMenu(message?: string): void {
    clearLiveState()
    resetEngine()
    setPhase('menu')
  ...
  function releaseMenuLock(): void {
    setBusyAction(null)
    const releaseTimer = window.setTimeout(() => {
      setPressedAction(null)
      menuLockRef.current = false
    }, MENU_RELEASE_MS)
    menuTimerRefs.current.push(releaseTimer)
  }

  function queueMenuAction(actionId: string, task: () => Promise<void> | void): void {
    if (menuLockRef.current || busyAction) {
      return
    }
    ...
    const pressTimer = window.setTimeout(() => {
      ...
      void Promise.resolve()
        .then(task)
        .catch((error: unknown) => {
          setErrorMessage(parseApiError(error, 'Islem tamamlanamadi.').message)
        })
        .finally(() => {
          releaseMenuLock()
        })
    }, MENU_PRESS_MS)
    menuTimerRefs.current.push(pressTimer)
  }
  ...
  function applyOverview(overview: YabuBirdLeaderboardResponse): void {
    ...
    menuRoomsRef.current = overview.live_rooms
  ...
  async function enterRoom(intent: JoinIntent): Promise<void> {
    ...
    setErrorMessage(null)
    setStatusMessage(intent.mode === 'HOST' ? 'ODA KURULUYOR...' : 'BAGLANTI KURULUYOR...')
    setPhase('joining')
    ...
    try {
      ...
      setStatusMessage(
        state.room.share_code
          ? `ODA ${state.room.share_code} HAZIR. DOKUN VE BASLA.`
          : 'DOKUN VE BASLA.',
      )
      setPhase('ready')
      ...
      resetToMenu('BAGLANTI KURULAMADI.')
  ...
  async function finishRun(snapshot: EngineState): Promise<void> {
    ...
    setStatusMessage(`SKOR ${snapshot.score}. TEKRAR DENE.`)
    playSfx('crash')
  ...
  async function leaveRoom(returnHome = false, silent = false): Promise<void> {
    ...
    try {
      ...
      applyOverview(overview)
      resetToMenu(silent ? undefined : 'ODADAN CIKILDI.')
    } catch (error) {
      if (!silent) {
  ...
  function handleStagePointerDown(event: ReactPointerEvent<HTMLDivElement>): void {
    event.preventDefault()
    ensureAudioContext()
    if (phaseRef.current === 'menu' || phaseRef.current === 'joining' || phaseRef.current === 'crashed') {
      return
    }
    flap()
  }
  ...
  useEffect(() => {
    menuRoomsRef.current = menuLiveRooms
  }, [menuLiveRooms])
  ...
  useEffect(() => {
    ...
    const intervalId = window.setInterval(() => {
      ...
      void updateYabuBirdLiveState({
        ...
        .catch((error) => {
          const parsed = parseApiError(error, 'Canli baglanti koptu.')
          if (parsed.code === 'YABUBIRD_ROOM_CLOSED' || parsed.code === 'YABUBIRD_ROOM_NOT_FOUND') {
            resetToMenu(parsed.message)
            void refreshOverview(true)
            return
          }
          setErrorMessage(parsed.message)
  ...
  useEffect(() => {
    ...
    const tick = (time: number) => {
      ...
      const renderElapsedMs =
        phaseRef.current === 'playing' ? engineRef.current.elapsedMs : (time * 0.85) % 120000
      const renderSeed =
        roomRef.current?.seed ??
        menuRoomRef.current?.seed ??
        menuRoomsRef.current.find((entry) => entry.room_type === 'PARTY')?.seed ??
        424242
      ...
      for (const pipe of getVisiblePipes(renderSeed, renderElapsedMs)) {
      ...
      renderedPlayers
        ...
        .forEach((player, index) => {
          drawBird(
            context,
            VIEW_WIDTH - 28 - index * 14,
            clamp(player.latest_y, 12, PLAY_HEIGHT - 20),
            player.color_hex,
            index % 3,
          )
        })
      ...
      const wingFrame =
        phaseRef.current === 'playing' ? Math.floor(time / 90) % 3 : Math.floor(time / 180) % 3
        ...
        >
          ...
          {room && you && (phase === 'ready' || phase === 'playing') ? (
            <>
              <button
                type="button"
                className="yabubird-exit-chip"
                onPointerDown={(event) => {
                  handleUiPointerDown(event)
                  void leaveRoom(false)
                }}
              >
                {leaveInFlightRef.current ? 'CIKILIYOR' : 'CIK'}
              </button>
          ...
          {phase === 'menu' ? (
            <div className="yabubird-boot-screen" onPointerDown={handleUiPointerDown}>
              ...
              {menuScreen === 'main' ? (
                <>
                  <div className="yabubird-boot-stack">
                    <button
                      type="button"
                      className={`yabubird-boot-option ${pressedAction === 'solo' ? 'is-pressed' : ''}`}
                      disabled={Boolean(busyAction)}
                      onPointerDown={handleMenuPointer('solo', () => enterRoom({ mode: 'SOLO' }))}
                    >
                      {busyAction === 'solo' ? 'YUKLENIYOR' : 'TEK OYNA'}
                    </button>
                    <button
                      type="button"
                      className={`yabubird-boot-option ${pressedAction === 'rooms' ? 'is-pressed' : ''}`}
                      disabled={Boolean(busyAction)}
                      onPointerDown={handleMenuPointer('rooms', () => {
                        setMenuScreen('rooms')
                        setStatusMessage('ACIK ODALARI SEC.')
                      })}
                    >
                      ODA LISTESI
                    </button>
                    <button
                      type="button"
                      className={`yabubird-boot-option ${pressedAction === 'host' ? 'is-pressed' : ''}`}
                      disabled={Boolean(busyAction)}
                      onPointerDown={handleMenuPointer('host', () => enterRoom({ mode: 'HOST' }))}
                    >
                      {busyAction === 'host' ? 'KURULUYOR' : 'ODA KUR'}
                    </button>
                    <button
                      type="button"
                      className={`yabubird-boot-option ${pressedAction === 'exit' ? 'is-pressed' : ''}`}
                      disabled={Boolean(busyAction)}
                      onPointerDown={handleMenuPointer('exit', () => {
                        navigate('/')
                      })}
                    >
                      CIKIS
                  ...
                  <div className="yabubird-boot-scoreline">
                    <span>REKOR {personalBest}</span>
                    <span>HI {leaderboard[0]?.score ?? 0}</span>
                  </div>
                ...
                <>
                  <div className="yabubird-room-list">
                    <div className="yabubird-room-line yabubird-room-line--header">
                      <span>KOD</span>
                      <span>DOLU</span>
                      <span>GIR</span>
                    </div>
                    ...
                    {roomList.length === 0 ? (
                      <div className="yabubird-room-line yabubird-room-line--empty">AKTIF ODA YOK</div>
                    ) : (
                      roomList.map((entry) => (
                        <button
                          key={entry.id}
                          type="button"
                          className={`yabubird-room-line yabubird-room-line--button ${pressedAction === `join-${entry.id}` ? 'is-pressed' : ''}`}
                          disabled={Boolean(busyAction)}
                          onPointerDown={handleMenuPointer(`join-${entry.id}`, () =>
                            enterRoom({ mode: 'ROOM', roomCode: entry.share_code }),
                          )}
                        >
                          <span>{`> ${entry.share_code ?? '------'}`}</span>
                          <span>{formatRoomSlots(entry)}</span>
                          <span>{busyAction === `join-${entry.id}` ? '...' : 'GIR'}</span>
                        </button>
                      ))
                    )}
                  </div>

                  <div className="yabubird-boot-footer">
                    <button
                      type="button"
                      className={`yabubird-boot-option ${pressedAction === 'back' ? 'is-pressed' : ''}`}
                      disabled={Boolean(busyAction)}
                      onPointerDown={handleMenuPointer('back', () => {
                        setMenuScreen('main')
                        setStatusMessage('TEK OYNA ya da ODA LISTESI sec.')
                      })}
                    >
                      GERI
                    </button>
          ...
          {phase === 'joining' ? (
            <div className="yabubird-boot-screen yabubird-boot-screen--center" onPointerDown={handleUiPointerDown}>
              <div className="yabubird-boot-loader">
                <p className="yabubird-boot-kicker">LINK</p>
                <h2>BAGLANIYOR</h2>
                <p>{statusMessage}</p>
                <div className="yabubird-boot-loader-dots" aria-hidden="true">
                  <span />
                  <span />
                  <span />
                </div>
          ...
          {phase === 'crashed' ? (
            <div className="yabubird-boot-screen yabubird-boot-screen--center" onPointerDown={handleUiPointerDown}>
              <div className="yabubird-boot-head">
                <p className="yabubird-boot-kicker">GAME OVER</p>
                <h1>YABU BIRD</h1>
                <p className="yabubird-boot-status">{statusMessage}</p>
              </div>
              ...
              <div className="yabubird-boot-stack">
                <button
                  type="button"
                  className={`yabubird-boot-option ${pressedAction === 'retry' ? 'is-pressed' : ''}`}
                  disabled={Boolean(busyAction)}
                  onPointerDown={handleMenuPointer('retry', () =>
                    enterRoom(joinIntentRef.current ?? { mode: 'SOLO' }),
                  )}
                >
                  TEKRAR
                </button>
                <button
                  type="button"
                  className={`yabubird-boot-option ${pressedAction === 'menu' ? 'is-pressed' : ''}`}
                  disabled={Boolean(busyAction)}
                  onPointerDown={handleMenuPointer('menu', async () => {
                    resetToMenu('TEK OYNA ya da ODA LISTESI sec.')
